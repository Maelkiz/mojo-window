...
def new_event_buffer(kind: UInt32) -> InlineArray[UInt8, SDL_EVENT_SIZE]:
    var buf = InlineArray[UInt8, SDL_EVENT_SIZE](fill=0)
    buf.unsafe_ptr().unsafe_bitcast[UInt32]()[] = kind
    return buf^


def write_u8(mut buf: InlineArray[UInt8, SDL_EVENT_SIZE], offset: Int, value: UInt8):
    buf.unsafe_ptr().unsafe_offset(offset).unsafe_bitcast[UInt8]()[] = value


def write_u32(mut buf: InlineArray[UInt8, SDL_EVENT_SIZE], offset: Int, value: UInt32):
    buf.unsafe_ptr().unsafe_offset(offset).unsafe_bitcast[UInt32]()[] = value


def write_i32(mut buf: InlineArray[UInt8, SDL_EVENT_SIZE], offset: Int, value: Int32):
    buf.unsafe_ptr().unsafe_offset(offset).unsafe_bitcast[Int32]()[] = value


def write_f32(mut buf: InlineArray[UInt8, SDL_EVENT_SIZE], offset: Int, value: Float32):
